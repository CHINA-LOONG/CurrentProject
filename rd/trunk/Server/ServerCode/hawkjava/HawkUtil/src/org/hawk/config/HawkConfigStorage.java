...
public class HawkConfigStorage {
	 ...
	 * 加载xml配置数据
	 */
	private void loadXmlConfig(Map<Object, HawkConfigBase> containerMap, List<HawkConfigBase> containerList) throws Exception {
		try {
			File file = new File(filePath);
			if (!file.exists()) {
				throw new HawkException("config file not exist: " + filePath);
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (attrMap != null) {
					...
					if (className != null && !"".equals(className)) {
						Object cfg = configType.newInstance();
						if (cfg instanceof HawkConfigBase) {
							...
							if(xmlNestRes != null){
								...
								} else if ("list".equals(xmlNestRes.struct())) {
									configBase.setConfigList(new LinkedList<HawkConfigBase>());
								} else {
