...
{
    NetCache * cache = NULL;
    if (rsize == NetCache::DEFAULT_READ_SIZE)
    {
        if (enable_pool)
        {
            cache = NetCacheManager::getInst()->getCache();
            if (cache)
            {
                cache->init(fd, addr);
            }
    ...
    if (cache == NULL)
    {
        cache = new NetCache(fd, addr, rsize);
    }
...
void NetHandler::removeConnection(int fd)
{
    ...
    if (cache != NULL)
    {
        //int64 id = cache->uid;
        fdCache.erase(fd);
        if (cache->getReadSize() == NetCache::DEFAULT_READ_SIZE)
        {
            if (enable_pool)
            {
                NetCacheManager::getInst()->RemoveCache(cache);
            }
            else
            {
                delete cache;
...
bool NetHandler::sendSizedTypeFdString(int fd, int type, const string& str)
{
    ...
    sendFdString(fd, (const char*) (&buf), sizeof (unsigned int) );
    bool succ = sendFdString(fd, str.c_str(), str.length());
    unlockSend();
    return succ;
}
            ...
            for (int i = 0; i <= fdmax; i++)
            {
                if (FD_ISSET(i, &readset))
                {
                    ...
                    else
                    {
                        ...
                        if (cache != NULL)
                        {
                            ...
                            if (readSucc)
                            {
                                ...
                                while (cache->assemble(req) && !cache->remove)
                                {
                                    ...
                                    {
                                        LOG4CXX_WARN(logger_, "Protocol handler is NULL for fd:" << i);
                                    }
                                }
                            }
                        ...
                        {
                            LOG4CXX_WARN(logger_, "Cannot find cache for fd:" << i);
                        }
                    }
                }
            }
...
void NetHandler::start()
{
    ...
    //int maxconn = (geteuid()==1) ? 100000 : 1000;
    int maxconn = 100000;
    struct rlimit srl;
