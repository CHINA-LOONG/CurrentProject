...
void
CSysLog::SetLogInfo(bool bGameLog, int nSrvID,
...
bool
CSysLog::CreateDir(const char* szDir)
{
    if (!TURNON_LOG)
        return true;
    if (!m_bShowLog)
...
void
CSysLog::EndMsg(int64 nUserID, LOG_TYPE emType, int nDmd)
{
    m_szBuf[m_nPos] = 0;
...
bool
CSysLog::IsStatUser(int64 nUserID)
{
    if (!m_bGameLog)
        return false;
    if (m_nModul == 0)
...
bool
CSysLog::NeedSend(int64 nUserID, LOG_TYPE emType, int nDmd)
{
    bool bSend = false;
    if (nDmd != 0)
        return true;
    switch (emType)
    {
        ...
        case LT_WebBuy:
        case LT_WebBuyDetail:
        case LT_LogTypeMustSend:
            bSend = true;
            break;
            //case LT_Connect:
            //case LT_Experience:
