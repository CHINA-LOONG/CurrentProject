...
BOOL DBCFile::OpenFromMemoryImpl_Text(const CHAR* pMemory, const CHAR* pDeadEnd, const CHAR* szFileName)
{
    ...
    //读第一行
    register const char* pMem = pMemory;
    for (pMem = _GetLineFromMemory(szLine, 1024 * 10, pMem, pDeadEnd);
            pMem != NULL && (szLine[0] == '#' || (szLine[0] == -1&& szLine[1] == -2));
    ...
    {
        _ConvertStringToVector(szLine + 3, vRet, "\t", TRUE, TRUE);
    }
    else
    {
    ...
    {
        if (vRet[i] == "INT" || vRet[i] =="int" || vRet[i] =="Int") vFieldsType[i] = T_INT;
        else if (vRet[i] == "FLOAT" ||vRet[i] =="float" || vRet[i]=="Float") vFieldsType[i] = T_FLOAT;
        else if (vRet[i] == "STRING"||vRet[i] =="string" || vRet[i]=="String") vFieldsType[i] = T_STRING;
        else
        {
            return FALSE;
        }
...
{
    assert(szFileName);
    std::cout << "DBC::OpenFromTxt [" << szFileName << "]" << endl;
    //----------------------------------------------------
    //打开文件
