...
public class BattleUnitAi{
	...
	{
		var  bossAi = battleUnit.battleUnit.GetBossAI();
		if (null != bossAi) 
		{
            if (bossAi.isUseXgAi)
            {
	...
	List<GameUnit> GetValidGameUnitList(List<BattleObject> battleList)
	{
		...
		for (int i =0; i<battleList.Count; ++i) 
		{
			...
			if(null!=subUnit && subUnit.unit.State != UnitState.Dead && subUnit.unit.isVisible)
			{
