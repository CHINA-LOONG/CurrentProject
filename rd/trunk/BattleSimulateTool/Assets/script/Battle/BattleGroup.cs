...
public class BattleGroup
{
    ...
    public BattleObject GetNextMoveUnit()
    {
        ...
        {
            var bo = enemyField[i];
            if (bo != null && bo.unit.dazhao > 0 && bo.unit.State != UnitState.Dead)
            {
                if (bo.unit.curLife <= 0)
                {
                    Logger.LogError("hahahahha,error life is 0");
                }
        ...
        {
            var bo = enemyField[i];
            if (bo != null && bo.unit.isVisible && bo.unit.dazhao == 0 && bo.unit.State != UnitState.Dead)
            {
                if (bo.unit.curLife <= 0)
                {
                    Logger.LogError("hahahahha,error life is 0");
