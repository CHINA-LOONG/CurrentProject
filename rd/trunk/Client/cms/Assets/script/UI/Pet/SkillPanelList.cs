...
public class SkillPanelList : PetDetailRight, SkillElementDelegate{
    ...
    private string m_currentSpellId;
    ...
    override public void ReloadData(GameUnit unit)
    {
        ...
        skillElements[(int)SkillIndex.SKILL_BUFF_INDEX].gameObject.SetActive(false);
    ...
    void ReloadElement(string spellId, Spell spell, GameUnit unit)
    {
        ...
        {
            skillElements[(int)SkillIndex.SKILL_PHY_INDEX].ReloadData(spellId, spell, unit);
        }
        ...
        {
            skillElements[(int)SkillIndex.SKILL_MAGIC_INDEX].ReloadData(spellId, spell, unit);
        }
        else if (spell.spellData.category == (int)SpellType.Spell_Type_Dot)
        {
            skillElements[(int)SkillIndex.SKILL_BUFF_INDEX].gameObject.SetActive(true);
            skillElements[(int)SkillIndex.SKILL_BUFF_INDEX].ReloadData(spellId, spell, unit);
        }
        else if (spell.spellData.category == (int)SpellType.Spell_Type_Passive)
        {
            skillElements[(int)SkillIndex.SKILL_PASSIVE_INDEX].ReloadData(spellId, spell, unit);
        }
        ...
        {
            skillElements[(int)SkillIndex.SKILL_DAZHAO_INDEX].ReloadData(spellId, spell, unit);
        }
    }
    ...
    IEnumerator IncreasePoint()
    {
        while (true)
        {
            ...
            {
                status.text = string.Format("{0} : {1}", StatisticsDataMgr.Instance.GetSkillPointLeftTime() / 60, StatisticsDataMgr.Instance.GetSkillPointLeftTime() % 60);          
            }
            else
            {
                status.text = @"技能点已满";  
