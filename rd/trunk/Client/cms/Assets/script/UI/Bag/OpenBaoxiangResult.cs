...
public class OpenBaoxiangResult : UIBase 
{
	public static string ViewName = "OpenBaoxiangResult";
	public	static	void	OpenWith(List<PB.HSRewardInfo> listReward)
	{
		OpenBaoxiangResult boxResult = (OpenBaoxiangResult)UIMgr.Instance.OpenUI_ (ViewName,false);
		boxResult.InitWith (listReward);
	}
	...
	void Start ()
	{
		...
		EventTriggerListener.Get (closeButton.gameObject).onClick = OnClose;
	...
	public	void	InitWith(List<PB.HSRewardInfo> listReward)
	{
		...
		for (int i =0; i < listReward.Count; ++ i)
		{
			...
			var boxReward = BaoxiangReward.CreateWith(i + 1,subRewardInfo);
			scrollView.AddElement(boxReward.gameObject);
		}
		scrollView.gameObject.SetActive (false);
		StartCoroutine ("openBoxAnimationCo");
	}

	IEnumerator	openBoxAnimationCo()
	{
