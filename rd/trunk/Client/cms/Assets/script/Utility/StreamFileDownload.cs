...
public class StreamFileDownload : MonoBehaviour
{
    ...
    public delegate void OnFinishDownloadDelegate(int state,string errMsg);
        ...
        switch (request.State)
        {
            ...
            case HTTPRequestStates.Finished:
                if (response.IsSuccess)
                {
                    ...
                    {
                        Logger.Log("Processing - continue");
                    }
                }
                else
                {
    ...
    void finishDownload(int state,string errmsg)
    {
        ...
        {
            errmsg = "file error";
            Logger.LogError("file error");
            state = -1;
        }
        finishCallBack(state, errmsg);
    }
}
