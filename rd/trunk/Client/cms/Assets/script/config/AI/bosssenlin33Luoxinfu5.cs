...
public class bosssenlin33Luoxinfu5 : BossAi {
	...
	public override BattleUnitAi.AiAttackResult GetAiAttackResult(GameUnit Luoxinfu5Unit)
	{
		...
		{
			if (wpList[n] == "bosssenlin33Luoxinfu5wp02")
				count++;
			if (wpList[n] == "bosssenlin33Luoxinfu5wp03")
				count++;
			if (wpList[n] == "bosssenlin33Luoxinfu5wp04")
				count++;
			if (wpList[n] == "bosssenlin33Luoxinfu5wp05")
				count++;
		}
	...
	//	if (args.wpID == "bossMinghe14Luoxinfu5wp03" && jishu==0)
	//  {
	//target.TriggerEvent("Luoxinfu5_state1to2", Time.time, null);
