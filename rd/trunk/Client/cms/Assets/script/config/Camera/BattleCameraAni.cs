...
public class BattleCameraAni 
{
	...
	{
		return	BattleCamera.Instance.cameraAni.MotionTo (BattleController.Instance.GetPhyDazhaoCameraNode(), 1.0f);
	}
	//运动到默认镜头模式
	public	static	Tweener	MotionToDefault()
	{
		return	BattleCamera.Instance.cameraAni.MotionTo (BattleController.Instance.GetDefaultCameraNode(), 1.0f);
	}
	//直接切换到默认镜头模式
