...
public class MonsterGrade : StaticDataBase 
{
	...
	private Dictionary<int,MonsterGrade.RowData> m_CacheData = new Dictionary<int, MonsterGrade.RowData>();

	public MonsterGrade.RowData getRowDataFromLevel(int iLevel)
	{
		...
		if (nRow < 0)
		{
			Debug.LogError("Can't find MonsterGradeE level = " +  iLevel);
			return null;
		}
		...
		subField = m_StaticTable.GetCertainField (nRow, "goldNoteValue");
		rd.goldNoteValue = int.Parse (subField.FieldValueStr);
		...
		subField = m_StaticTable.GetCertainField (nRow, "expValue");
		rd.expValue = int.Parse (subField.FieldValueStr);
